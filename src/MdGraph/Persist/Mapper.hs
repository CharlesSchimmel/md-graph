...
import MdGraph.File.Types (RelativePath (..))
import qualified MdGraph.File.Types as File
...
fromNodeLink docKey Node.Link {linkPath, linkText} =
  Edge
    { edgeTail = docKey,
      edgeHead = linkPath,
...
instance ToEdge RelativeLink where
  toEdge docKey (RelativeLink Node.Link {linkPath, linkText}) =
    Edge
      { edgeTail = docKey,
        edgeHead = linkPath,
        edgeLabel = T.unpack linkText
